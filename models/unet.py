    ...
    def __init__(self, in_channels, num_classes, first_out_channels=32):
        ...
        with torch.no_grad():
            self.projector = nn.Sequential(
                nn.Conv3d(in_channels=8 * in_channels, out_channels=1024, kernel_size=1),
                nn.BatchNorm3d(1024),
                nn.ReLU(inplace=True),
                nn.Conv3d(in_channels=1024, out_channels=8 * in_channels, kernel_size=1),
            )
